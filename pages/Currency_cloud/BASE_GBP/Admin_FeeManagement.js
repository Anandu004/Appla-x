...
exports.Admin_FeeManagement =
    class Admin_FeeManagement {
        ...
        async getReferenceNumberFromClient(page) {
            ...
            const gbp_referenceNo = (await this.page.locator("//table/tbody/tr[1]/td[4]//span").textContent())?.trim()
            console.log(" Extracted Reference No:", gbp_referenceNo)
            ...
            data.gbp_referenceNo = gbp_referenceNo
        ...
        async getNew_Walletbalance(page) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (firstThree === 'gbp') {
                    ...
                    console.log("Actual New balance in the wallet (GBP):", new_wallet_balance)
        ...
        async verify_Tariff_Fee_deduction_for_Transaction_Investiagtion(page) {
            ...
            const { gbp_referenceNo } = JSON.parse(fs.readFileSync(referencePath, 'utf-8'))

            console.log(" Reference retrieved inside method:", gbp_referenceNo)
            ...
            if (refFieldVisible) {
                console.log("Entering reference number:", gbp_referenceNo)
                await this.page.fill(this.reference_no, gbp_referenceNo)
            } else {
                throw new Error("Reference Number not required for this fee type.")
            }
            ...
            await this.page.fill(this.search_record, gbp_referenceNo)
            ...
            if (rowCount === 0) {
                 throw new Error(` No record found for Reference No: ${gbp_referenceNo}`)
            }
            ...
            expect(ref_no).toBe(gbp_referenceNo)
            ...
            //  No rows found → throw error
            if (log_rowCount === 0) {
                 throw new Error(` No record found in Activity log for Reference No: ${gbp_referenceNo}`)
            }
            ...
            expect(actionMessage).toContain(gbp_referenceNo)
        ...
        async verify_Tariff_Fee_deduction_for_AnnualAudit_Reports(page) {
            ...
            // Extract values from Tariff fee deduction list
            //const firstRow = await this.page.locator("//table/tbody/tr[1]")
            const rows = this.page.locator(`//table/tbody/tr[td[contains(text(),"${feeTypeToApply}")]and td[contains(text(),"${company_name}")] ]`)
            ...
            await this.page.waitForTimeout(4000)
            //const log_Row = await this.page.locator("//table/tbody/tr[1]")
            const row = this.page.locator(`//table/tbody/tr[td[contains(text(),"${feeTypeToApply}")]and td[contains(text(),"${company_name}")] ]`)
            const log_Row = row.first()
            const module = (await log_Row.locator("td:nth-child(3)").textContent()).trim()
            const actionMessage = (await log_Row.locator("td:nth-child(4)").textContent()).trim()
